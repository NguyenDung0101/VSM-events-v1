...
import { FileText, Calendar, Settings, Eye, Edit, Camera, User } from "lucide-react"
...
export default function ProfilePage() {
  ...
  // Get user's posts (for editor/admin)
  const userPosts = mockPosts.filter((post) => post.authorId === user.id)

  // Get user's registered events
  const userRegistrations = mockEventRegistrations.filter((reg) => reg.userId === user.id)
  const registeredEvents = mockEvents.filter((event) => userRegistrations.some((reg) => reg.eventId === event.id))
      ...
      <main className="pt-16">
        <section className="py-12 bg-gradient-to-r from-primary/20 to-purple-500/20">
          <div className="container mx-auto px-4">
            ...
            >
              <h1 className="text-4xl md:text-5xl font-bold mb-6">Hồ sơ cá nhân</h1>
              <p className="text-xl text-muted-foreground">Quản lý thông tin cá nhân và theo dõi hoạt động của bạn</p>
            </motion.div>
          </div>
        </section>
                        ...
                        <DialogContent>
                          ...
                          <div className="grid grid-cols-4 gap-4 p-4">
                            {avatarOptions.map((avatar, index) => (
                              <button key={index} onClick={() => handleAvatarChange(avatar)} className="relative group">
                                <img
              ...
              <Tabs defaultValue="info" className="space-y-6">
                <TabsList className="grid w-full grid-cols-3">
                  ...
                  <TabsTrigger value="posts">
                    {user.role === "admin" || user.role === "editor" ? "Bài viết" : "Hoạt động"}
                  </TabsTrigger>
                      ...
                      {isEditing ? (
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <p className="text-lg">{getRoleText(user.role)}</p>
                          </div>
                          <div>
                    ...
                    <CardContent>
                      {user.role === "admin" || user.role === "editor" ? (
                        userPosts.length === 0 ? (
                          <div className="text-center py-8">
                            ...
                            <Button className="mt-4" asChild>
                              <Link href="/dashboard/posts">Tạo bài viết đầu tiên</Link>
                            </Button>
                          </div>
                        ) : (
                          <div className="space-y-4">
                            {userPosts.map((post) => (
                              <div key={post.id} className="flex items-center space-x-4 p-4 border rounded-lg">
                                <img
                                  src={post.cover || "/placeholder.svg?height=64&width=64"}
                                  alt={post.title}
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-muted-foreground">Hoạt động của bạn sẽ hiển thị ở đây.</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                    ...
                    <CardContent>
                      {registeredEvents.length === 0 ? (
                        <div className="text-center py-8">
                          ...
                          <p className="text-muted-foreground">Bạn chưa đăng ký sự kiện nào.</p>
                          <Button className="mt-4" asChild>
